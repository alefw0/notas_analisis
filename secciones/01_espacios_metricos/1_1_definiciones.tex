...
\begin{definicion}Sea $X\neq \emptyset$. \begin{enumerate}
    \item En este libro $\N$ denota el conjunto $\{1,2,3,\dots\}$ de los números naturales. Una sucesión en $X$ es una función $\x:\N \to X$, usualmente denotada como $\left\{ x_n \right\}_{n=1}^\infty $ o  $\left\{ x_n \right\} .$ También se puede considerar como sucesión a una función cuyo dominio es $\left\{ k, k+1, \dots\right\} \subset \N$  y se denotará  $\left\{ x_n \right\}_{n=k}^\infty $.
    \item Sea $X=\mathbb{R}$. Decimos que la sucesión es acotada si existe un real $M$ tal que $|x_n|\leq M$, para todo $n \in \N.$
    \item Denotemos por $B(\N)$ al conjunto de todas las sucesiones acotadas en $\mathbb{R}$.
\end{enumerate}
\end{definicion}


\begin{example}
La función $d:B(\N)\times B(\N) \to \mathbb{R}$, definida por $d(\x , \y):=\sup\left\{ |x_n-y_n|: \ \ n\in \N \right\},$ donde $\x=\left\{x_n \right\}$ y $\y=\left\{y_n \right\}$, es una métrica en $B(\N).$
\end{example}
\begin{proof}
Sean $\x=\{x_n\}$, $\y=\{y_n\}$ y $\z=\{z_n\}$ sucesiones en $B(\N)$. Notemos que esta función está bien definida. En efecto, existen reales $M_1$ y $M_2$ tales que  $|x_n|\leq M_1$ y $|y_n|\leq M_2$ para todo $n$. Luego $|x_n-y_n|\leq |x_n|+|y_n|\leq M_1+M_2$, para todo $n$. En consecuencia, el conjunto $\{|x_n-y_n|:\ \ n\in \N\}$ es acotado y por tanto el supremo existe en $\R$.
\begin{enumerate}
    \item Claramente $d(\x, \y) \geq 0$.  
    \item Además $d(\x, \y)=0$ si y sólo si $|x_n-y_n|=0$ para todo $n\in \N$; es decir ; $\x=\y$.
...
\end{teorema}
\begin{proof}
Sea $\{x_n\}$ una sucesión convergente en $X$ y sea $x$ su límite. Existe un entero positivo $N$ tal que para todo $n\geq N$, $d(x_n, x)<1.$ Sea $M=\max\{d(x_1,  x), \dots , d(x_{N-1},  x), \, 1\}.$ Entonces para todo $n\in \N$, $x_n\in B(x, M).$
\end{proof}
...
\begin{definicion}
Sea $\{a_n\}_{n\in\N}$ una sucesión.  
Se llama \emph{subsucesión} de $(a_n)$ a toda sucesión de la forma
\[
\{a_{n_k}\}_{k\in\N}
\]
donde $\{n_k\}_{k\in\N}$ es una sucesión estrictamente creciente de números naturales, es decir,
\[
n_1 < n_2 < n_3 < \cdots
\]
\end{definicion}
...
\begin{teorema}
    Si $(X,d)$ es un espacio métrico y $\left\{x_n\right\}$ es una sucesión en $X$ convergente a $x$, entonces cada subsucesión de $\left\{x_n\right\}$ converge a $x$.
\end{teorema}
\begin{proof}
...
\[
\forall \varepsilon > 0,\ \exists N \in \N \text{ tal que } d(x_n,x) < \varepsilon
\quad \text{para todo } n \ge N.
\]

Sea $\{x_{n_k}\}$ una subsucesión de $\{x_n\}$.  
Como la sucesión de índices $\{n_k\}$ es estrictamente creciente, existe $k_0 \in \N$ tal que $k_0\ge N$.Dado que $n_k\ge k$ tenemos que
\[
