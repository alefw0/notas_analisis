...
\begin{definicion}
 ...
 \item Si $U$ es un abierto y $f$ es diferenciable en cada punto $c\in U$, decimos que $f$ es diferenciable (en $U$). La función $f':U\to \mathbb{R}$, se llama la  derivada de $f$.
...
\[f'(x)= \left\{ \begin{array}{rll}
    2x\sen\left(\frac{1}{x}\right)-\cos\left(\frac{1}{x}\right), & \text{si}  & x\neq 0  \\
    0, & \text{si} & x=0 
 \end{array}\right. .\]
...
\[(f \pm g)'(c) = \lim_{x \to c} f^*(x) \pm \lim_{x \to c} g^*(x) \quad \text{Propiedad del límite}\]
...
\begin{itemize}
    \item $f^*$ es continua en $c$ por el teorema 1
