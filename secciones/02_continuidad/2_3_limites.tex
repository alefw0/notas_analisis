...
\[
(\exists\epsilon>0)(\forall\delta>0)(\exists x_\delta\in A\setminus\{p\})\;
d_X(x_\delta,p)<\delta \ \land\ d_Y(f(x_\delta),b)\ge\epsilon.
\]
\end{remark}
...
\begin{definicion}
\begin{enumerate}
    \item Decimos que $\lim_{x\to p^+}f(x)=b$ si para todo $\epsilon>0$ existe $\delta>0$ tal que, si $x\in A$ y $p<x<p+\delta$, entonces $|f(x)-b|<\epsilon$.
    \item Decimos que $\lim_{x\to p^-}f(x)=b$ si para todo $\epsilon>0$ existe $\delta>0$ tal que, si $x\in A$ y $p-\delta<x<p$, entonces $|f(x)-b|<\epsilon$.
\end{enumerate}
\end{definicion}
