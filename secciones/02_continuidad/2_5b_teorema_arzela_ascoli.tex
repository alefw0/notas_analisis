...
\begin{enumerate}
    \item $\mathfrak{F}$ es \emph{equicontinua} si para todo $\varepsilon>0$ existe $\delta>0$ tal que
    \[
...
Ahora probamos equicontinuidad. Fijado $\varepsilon>0$, como $\mathfrak{F}$ es precompacto, es totalmente acotado, así que existen
$f_1,\dots,f_m\in\mathfrak{F}$ con
\[
\mathfrak{F}\subset\bigcup_{i=1}^m B_{\varepsilon/3}(f_i)
\]
en $d_\infty$.
Cada $f_i$ es uniformemente continua en $X$ (Heine), por lo que existe $\delta>0$ tal que
\[
d_X(x,x')<\delta \implies d_Y(f_i(x),f_i(x'))<\varepsilon/3
\]
para todo $i=1,\dots,m$.

Sea $f\in\mathfrak{F}$ y $x,x'\in X$ con $d_X(x,x')<\delta$. Tomamos $j$ tal que
$d_\infty(f,f_j)<\varepsilon/3$. Entonces
\[
d_Y(f(x),f(x'))
...
Fijamos $\varepsilon>0$. Por equicontinuidad de $\mathfrak{F}$, existe $\delta>0$ tal que
\[
d_X(x,x')<\delta \implies d_Y(h(x),h(x'))<\frac{\varepsilon}{3}
...
\[
i,j\ge N_r \implies d_Y(g_i(\hat x_r),g_j(\hat x_r))<\frac{\varepsilon}{3}.
\]
Definimos
\[
N:=\max\{N_1,\dots,N_m\}.
...
\[
d_Y(g_i(\hat x_r),g_j(\hat x_r))<\frac{\varepsilon}{3}
...
\[
d_\infty(g_i,g_j)<\varepsilon \quad (i,j\ge N).
\]
...
\[
d_Y(g_i(x),g(x))\le\varepsilon \quad (\forall x\in X,\ i\ge N),
\]
y por tanto
\[
d_\infty(g_i,g)=\sup_{x\in X}d_Y(g_i(x),g(x))\le\varepsilon.
\]
